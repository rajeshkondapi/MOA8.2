...
public class HomePageAPI extends CommonMethods {
	...
	Map<String,List<Schedule>> popuporders = new HashMap<String,List<Schedule>>();
	...
	public String ETLiveTranBody(String ETLiveTrans, String TopSendPin) throws Exception {
		...
		try {
			...
			else if (ETLiveTrans == "GetPopupInfo") {
				ETLiveTran.put("lang", "1");
				ETLiveTran.put("channel", "odp");
				
			}
			else if (ETLiveTrans == "chkHeader") {
				ETLiveTran.put("lang", "1");
				ETLiveTran.put("channel", "odp");
				
			}
			else if (ETLiveTrans == "getbanners") {
				ETLiveTran.put("lang", "1");
				ETLiveTran.put("channel", "odp");
				ETLiveTran.put("bid", "31");
				ETLiveTran.put("width", "100");
				ETLiveTran.put("height", "100");
				ETLiveTran.put("imsi", "0000000000");
				ETLiveTran.put("reqtype", "banners");				
	...
	public List<String> ETLiveTransApi(String ETLiveTr, String ProTran, String Uid, String Ver, String resppath,
			String uri, String pathext, String TopSendPin, String respnext) throws JSONException, InterruptedException, Exception {
		...
		if(ETLiveTr.trim().equalsIgnoreCase("MpitesenPayment")) {
			...
			// System.out.println("5");
			request.header("X-IMI-AUTHKEY", "APPYM8205D04");
			if(ETLiveTr == "chkHeader") {
				System.out.println("Checkheader");
				request.header("X-IMI-VERSION", "7.4.0");
				request.header("MSISDN", rc4.encrypt("959971357267"));
			}
			request.body(ETLiveTranBody(ETLiveTr, TopSendPin));
			// System.out.println("7");
			request.header("X-IMI-HASH", SessionHashKeyETLiveTrans(ETLiveTranBody(ETLiveTr, TopSendPin)));
			...
			try {
				Object w = (responseETL.body().jsonPath().get(resppath) != null
						...
						/*Object pop = (responseETL.body().jsonPath().get(resppath+".data") != null
								&& !responseETL.body().jsonPath().get(resppath).equals(""))
								? responseETL.body().jsonPath().get(resppath)
										: new ArrayList<Map<String, Object>>();
								*/
								...
								/*List<Map<Object,Object>> featureServices =  (List<Map<Object, Object>>) w;
								for (Object featureService : featureServices) {
								    for (Map.Entry<String, Object> entry : ((HashMap<String,Object>) featureService).entrySet()) {
								       System.out.println(entry.getKey() + ": " + entry.getValue());								       
								       if(entry.getKey().equals("data")) {
								    	   System.out.println("yes iter");
								       }
								    }*/
						...
						if (w != null) {
							...
							if (w instanceof List) {
								for (int h = 0; h < ((List<Map<String, Object>>) w).size(); h++) {
								System.out.println("in list object");	
								String vasdetails = responseETL.body().jsonPath().get(resppath +"[" + h + "]." + pathext);
								//String vasdetails1 = responseETL.body().jsonPath().get(resppath +"." + respnext);//							
								System.out.println("Details =>"+vasdetails);
								apirespETL.add(vasdetails);
								...
								if(w instanceof Map) {
									...
									String json = "";
									//json = FileUtils.readFileToString(new File("C:\\Users\\ravi.a\\Downloads\\response.txt"));
									json = responsebody;
									int dataLength = JsonPath.read(json, "$.popups.data.length()");
									if (dataLength > 0) {
										for (int i = 0; i < dataLength; i++) {
											String order = JsonPath.read(json, "$.popups.data[" + i + "].order");
											String name = JsonPath.read(json, "$.popups.data[" + i + "].name");
											int scheduleLen = JsonPath.read(json, "$.popups.data[" + i + "].schedule.length()");
											List<Schedule> list = new ArrayList<Schedule>();
											if (scheduleLen > 0) {
												for (int j = 0; j < scheduleLen; j++) {
													Schedule schedule = new Schedule();
													String userStatArr = (getString(json,
															"$.popups.data[" + i + "].schedule[" + j + "].userstatus", i, j) != null
															&& !getString(json, "$.popups.data[" + i + "].schedule[" + j + "].userstatus", i, j)
																	.isEmpty()) ? getString(json,
																			"$.popups.data[" + i + "].schedule[" + j + "].userstatus", i, j)
																			: "";
													String scheduleName = getString(json, "$.popups.data[" + i + "].schedule[" + j + "].name",
															i, j);
													schedule.setUserstatus(userStatArr);
													schedule.setName(scheduleName);
													list.add(schedule);
												}
											}
										
											String key1 = order + "_" + name;
											popuporders.put(key1, list);
									}
	...
	public static String getString(String json, String path, int i, int j) {
		try {
